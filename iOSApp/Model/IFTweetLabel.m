...
- (void)handleButton:(id)sender
{
    ...
    if ([firstCharacter isEqualToString:@"@"]) {
        [[NSUserDefaults standardUserDefaults] setObject:buttonTitle forKey:IFTweetLabelButtonTitle];
        
        [[NSNotificationCenter defaultCenter] postNotificationName:IFTweetLabelUserNotification object:nil];
        
    }else if ([firstCharacter isEqualToString:@"#"]) {
        [[NSUserDefaults standardUserDefaults] setObject:buttonTitle forKey:IFTweetLabelButtonTitle];
        
        [[NSNotificationCenter defaultCenter] postNotificationName:IFTWeetLabelPostNotification object:nil];
    }
}
...
- (void)createButtonWithText:(NSString *)text withFrame:(CGRect)frame
{
	...
	[button setFrame:CGRectMake(frame.origin.x - 0.3, frame.origin.y + 3, frame.size.width, frame.size.height)];
    ...
    NSRange matchRange = [self.label.text rangeOfString:text options:NSLiteralSearch range:NSMakeRange(0, [self.label.text length])];
    
    [attrString addAttribute:NSForegroundColorAttributeName value:buttonFontColor range:matchRange];
...
- (void)createButtonsWithText:(NSString *)text atPoint:(CGPoint)point
{
	...
	for (NSString *expression in expressions)
	{
		...
		while (match = [enumerator nextObject])
		{
			...
			CGRect matchFrame = CGRectMake(measureSize.width, point.y, matchSize.width, matchSize.height);
...
#define MIN_WHITESPACE_LOCATION 0
...
- (id)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame])
	{
        ...
        [self.label drawTextInRect:UIEdgeInsetsInsetRect(CGRectMake(frame.origin.x, frame.origin.y, frame.size.width, frame.size.height), UIEdgeInsetsMake(0, 0, 0, 0))];
    }
...
- (void)setText:(NSString *)text
{
    ...
    [self setFrame:[self setDynamicHieghtForLabel:self.label andMaxWidth:270]];
