...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _mainButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0,screenSize.width, 40)];
        ...
        [_mainButton setTitleEdgeInsets:UIEdgeInsetsMake(0, 27, 0, 27)];
        [self addSubview:_mainButton];
    }
    return self;
}

+(MTDropDownView *)showDropDownViewInView:(UIView *)view text:(NSString *)text animated:(BOOL)animated target:(id)target selector:(SEL)selector hideAfter:(float)hideAfter {
    CGSize screenSize = [[UIScreen mainScreen] bounds].size;
    MTDropDownView *dropDownView = [[MTDropDownView alloc] initWithFrame:CGRectMake(0, -44, screenSize.width, 44)];
    ...
    [dropDownView setFrame:CGRectMake(0, 0 - size.height, screenSize.width, size.height+22)];
