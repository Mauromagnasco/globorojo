...
- (void)setup {
    ...
    prevAngle = 100;
	self.minimumTrackTintColor = [UIColor colorWithRed:0.92 green:0.1 blue:0.14 alpha:1];
...
#pragma mark - Drawing methods
#define kLineWidth 45.0
#define kThumbRadius 0.0
- (CGFloat)sliderRadius {
	CGFloat radius = MIN(self.bounds.size.width/2, self.bounds.size.height/2) + 19;
...
- (void)panGestureHappened:(UIPanGestureRecognizer *)panGestureRecognizer {
	...
	switch (panGestureRecognizer.state) {
		case UIGestureRecognizerStateChanged: {
            ...
            NSLog(@"Prev_Angle - %f", prevAngle);
            NSLog(@"B_Angle - %f", angle);
            
            if (prevAngle == 100) {
                prevAngle = angle;
            }else {
                if (prevAngle > angle) {
                    if (prevAngle > 3 * M_PI / 2 && angle < M_PI / 2) {
                        angle = 2 * M_PI;
                        prevAngle = angle;
                        break;
                    }
