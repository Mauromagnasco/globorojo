...
#define REQUEST_FOR_FOLLOWERS               @"followers"
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@"Follow" forKey:@"lastView"];
    
    [[NSUserDefaults standardUserDefaults] setObject:@"Followers" forKey:@"currentView"];
    
    [[NSNotificationCenter defaultCenter]
     addObserver:self selector:@selector(requestFollowUserId:) name:IFNotiLabelFollowUserNotification object:Nil];
    
    followsItemArray = [[NSMutableArray alloc] init];
...
#pragma mark - 
#pragma mark SetStyle Functions 
...
- (void) requestFollowUserId: (NSNotification *)notification {
...
- (void) requestFollowers {
    ...
    NSMutableString *urlString = [NSMutableString stringWithFormat:@"%@%@", SERVER_HOST, FOLLOW_LIST_URL];
    ...
    //set Request Type;
    requestType = REQUEST_FOR_FOLLOWERS;
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if ([successFlag isEqualToString:TAG_SUCCCESS]) {
        ...
        if ([requestType isEqualToString:REQUEST_FOR_NOTIFICATION]){
        ...
        }else if ([requestType isEqualToString:REQUEST_FOR_FOLLOWERS]) {
            
            followsItemArray = [NSMutableArray array];
            ...
            for (int i = 0; i < [userListArray count]; i++) {
                ...
                [followsItemArray addObject:fiVC];
                
                FriendItemViewController *vc = (FriendItemViewController *)[followsItemArray objectAtIndex:[followsItemArray count] - 1];
