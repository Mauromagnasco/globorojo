 ...
 */
	...
	$sql = "
		select t1.*, t2.rb_username, t2.rb_photo, t2.rb_cred
		 ...
		 where t1.rb_user = t2.rb_user
		   and ( t1.rb_user in (select rb_following from rb_friend where rb_follower = $userId) or (t1.rb_user = $userId) )
		   and t1.rb_created_time < '$currentTime'";
	if( $period == 1 && $type == 1 ){
		$sql = " and date(t1.rb_created_time) = date( now() )";
	}else if( $period == 2 && $type == 1 ){
		$sql = " and t1.rb_created_time > concat(date(date_sub(now(), interval weekday( now( ) ) day)),' 00:00:00')";
	}	
	if( $type == 0 )
	...
	$firstYn = $dataResult[0]['rb_first_yn'];
	
	if( $firstYn == "Y" ){
		$sql = "update rb_user set rb_first_yn = 'N' where rb_user = $userId";
		$db->query( $sql );
	}
	
	$data['firstYn'] = $firstYn;
