 ...
 */
    ...
    $userId = mysql_escape_string($_POST['userId']); // 0 : date, 1 : score, 2 : cred
    ...
    $sql = "select t1.*, if( ifnull( t2.rb_friend, 'N' ) = 'N', 'N', 'Y' ) as isFollowing
    		 ...
    		 where ( t1.rb_user in (select rb_following from rb_friend where rb_follower = $userId) or (t1.rb_user = $userId) )";
    if( $txtKeyword == "" ){

    }else{
		$sql.= " and t1.rb_username like '%$txtKeyword%'";
	}
	if( $type == 1 )
